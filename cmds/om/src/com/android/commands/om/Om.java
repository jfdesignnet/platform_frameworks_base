    ...
    private int runSetPriority() {
        ...
        try {
            ...
            if ("lowest".equals(moveTo)) {
                return mOm.setLowestPriority(mOm.getOverlayInfo(packageName, userId)) ? 0 : 1;
            } else if ("highest".equals(moveTo)) {
                return mOm.setHighestPriority(mOm.getOverlayInfo(packageName, userId)) ? 0 : 1;
            } else {
                return mOm.setPriority(mOm.getOverlayInfo(packageName, userId), moveTo) ? 0 : 1;
            }
        } catch (RemoteException e) {
            System.err.println(e.toString());
            System.err.println(OM_NOT_RUNNING_ERR);
