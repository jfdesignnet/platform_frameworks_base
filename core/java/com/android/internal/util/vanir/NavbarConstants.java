...
public class NavbarConstants {
    ...
    public interface AwesomeGuts {
        ...
        //returns the resource id of this NavbarConstant's user-facing name
        int getStringId();

        //getStrings ^
        String getProperName(Context mContext);

        //gets a Resources where the drawable can be found (i.e. from mDrawablePackage)
        Resources getMyPackageResources(Context context);

        //gets the identifier from ^
        int getDrawableId(Context context);

        //gets the drawable from ^ and ^^
        Drawable getDrawable(Context mContext);
    }
